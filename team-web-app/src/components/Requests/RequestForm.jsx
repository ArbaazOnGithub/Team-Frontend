        ...
        <div className={`glass-card bg-white/60 p-6 mb-8 border-2 transition-colors ${isOverBalance ? 'border-rose-300' : 'border-[#547792]/20'}`}>
            <div className="flex gap-4 mb-4">
                <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${requestType === 'General' ? 'bg-[#68BA7F] text-white' : 'bg-black/30 text-[#68BA7F]/60 hover:bg-white'}`}
                >
                    💬 General Query
                </button>
                <button
                    onClick={() => setRequestType('Leave')}
                    className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${requestType === 'Leave' ? 'bg-[#68BA7F] text-white' : 'bg-black/30 text-[#68BA7F]/60 hover:bg-white'}`}
                >
                    📅 Leave Request
                </button>
            </div>
            ...
            {requestType === 'Leave' && (
                <div className="mb-4 animate-fadeIn">
                    <div className="flex justify-between items-center mb-4 px-2">
                        ...
                        <span className="text-xs font-black text-[#68BA7F] bg-[#94B4C1] px-3 py-1 rounded-full border border-[#547792]/30 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div className="space-y-1">
                            <label className="text-[10px] font-black text-[#68BA7F] uppercase ml-1">From Date</label>
                            <input
                                type="date"
                                value={startDate}
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-black text-[#68BA7F] uppercase ml-1">To Date</label>
                            <input
                                type="date"
                                value={endDate}
                    ...
                    {daysCount > 0 && (
                        <div className={`flex items-center justify-between p-3 rounded-xl mb-4 border ${isOverBalance ? 'bg-rose-50 border-rose-200' : 'bg-[#94B4C1]/40 border-[#547792]/20'}`}>
                            <div className="flex items-center gap-2">
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-wider ${isOverBalance ? 'text-rose-600' : 'text-[#68BA7F]'}`}>
            ...
            <div className="flex flex-col gap-2 mb-4">
                <textarea
                    ...
                    className="input-premium min-h-24 text-[#68BA7F] placeholder:text-[#68BA7F]/90 border-dashed border-[#547792]/40"
                    maxLength={1000}
                />
                ...
                <div className="flex items-center gap-3">
                    <button 
                        ...
                        onClick={takePhoto}
                        disabled={uploading}
                        className="flex items-center gap-2 px-4 py-2 bg-white/60 border border-[#547792]/20 rounded-xl text-[10px] font-black text-[#68BA7F] uppercase tracking-widest hover:bg-white transition-all disabled:opacity-50"
                    >
            ...
            <div className="flex justify-between items-center">
                ...
                <button
                    ...
                    className={`btn-premium px-8 py-3 rounded-xl font-bold text-sm transition-all shadow-lg ${isOverBalance ? 'bg-rose-500 hover:bg-rose-600 shadow-rose-200 text-white' : 'bg-[#68BA7F] hover:bg-[#68BA7F] shadow-[#68BA7F]/20 text-white'}`}
                >

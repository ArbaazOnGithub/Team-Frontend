        ...
        <div className="animate-fadeIn">
            ...
            <div className="flex gap-4 mb-6 border-b border-[#547792]/10 pb-4">
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'requests' ? 'bg-[#68BA7F] text-white shadow-md' : 'text-[#68BA7F]/60 hover:bg-white'}`}
                >
                    Ticket Logs
                </button>
                <button
                    onClick={() => setActiveSubTab('system')}
                    className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'system' ? 'bg-[#68BA7F] text-white shadow-md' : 'text-[#68BA7F]/60 hover:bg-white'}`}
                >
                    System Activity
                </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex flex-1 gap-4 max-w-2xl">
                    <div className="relative group flex-1">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#68BA7F]/60 group-focus-within:text-[#68BA7F] transition-colors">🔍</span>
                        <input
                            type="text"
                            placeholder="Search by Request No, Name, or Query..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="input-premium pl-12 py-3 bg-black/20 border-[#68BA7F]/30 focus:bg-white"
                        />
                    </div>
                    <select
                        value={filterStatus}
                        onChange={(e) => setFilterStatus(e.target.value)}
                        className="input-premium w-40 py-3 bg-black/20 border-[#68BA7F]/30 font-bold text-xs uppercase tracking-widest cursor-pointer focus:bg-white text-[#68BA7F]"
                    >
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-[#68BA7F] text-white rounded-xl font-bold text-sm hover:bg-[#68BA7F] shadow-lg shadow-[#68BA7F]/20 transition-all active:scale-95"
                >
                    📥 Export to Excel
                </button>
            </div>
            ...
            <div className="glass-card overflow-hidden border-white/5">
                        ...
                        <tbody>
                            <AnimatePresence mode="popLayout">
                                {filteredLogs.length > 0 ? filteredLogs.map((log) => (
                                    ...
                                    >
                                        <td className="p-5 font-black text-[#68BA7F]">#{log.requestNo || '---'}</td>
                                        <td className="p-5">
                                            <div className="font-bold text-[#68BA7F]">{log.user?.name || 'Deleted User'}</div>
                                            <div className="text-[10px] text-[#547792]">{log.user?.email}</div>
                                        </td>
                                        <td className="p-5 max-w-xs">
                                            <p className="text-xs text-[#68BA7F]/80 line-clamp-2">{log.query}</p>
                                        </td>
                                        <td className="p-5">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${log.status === 'Pending' ? 'bg-[#547792]/10 text-[#547792]' :
                                                log.status === 'Approved' ? 'bg-[#68BA7F]/10 text-[#68BA7F]' :
                                                    log.status === 'Resolved' ? 'bg-[#68BA7F]/10 text-[#68BA7F]' :
                                        ...
                                        <td className="p-5">
                                            {log.actionBy ? (
                                                <div className="flex items-center gap-2">
                                                    <span className="w-2 h-2 rounded-full bg-[#68BA7F]"></span>
                                                    <span className="text-xs font-bold text-[#68BA7F]">{log.actionBy.name}</span>
                                                </div>
                                            ) : (
                                        ...
                                        <td className="p-5">
                                            <div className="text-[10px] font-bold text-[#68BA7F]">{new Date(log.createdAt).toLocaleDateString()}</div>
                                            <div className="text-[9px] text-[#547792]">{new Date(log.createdAt).toLocaleTimeString()}</div>
                                        </td>
                                    </motion.tr>
                                )) : !loading && (
                                    <tr>
                                        <td colSpan="6" className="p-10 text-center font-bold text-[#68BA7F]/40">No logs matching your criteria</td>
                                    </tr>
                                )}
                            </AnimatePresence>
                        </tbody>
                ...
                {loading && (
                    <div className="p-10 text-center">
                        <div className="animate-spin w-8 h-8 border-4 border-[#68BA7F] border-t-transparent rounded-full mx-auto mb-4"></div>
                ...
                <div className="glass-card overflow-hidden border-white/5 mt-8">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-black/5 bg-white/5">
                                    <th className="p-5 text-[#68BA7F] font-bold uppercase text-[10px] tracking-widest">User</th>
                                    <th className="p-5 text-[#68BA7F] font-bold uppercase text-[10px] tracking-widest">Action</th>
                                    <th className="p-5 text-[#68BA7F] font-bold uppercase text-[10px] tracking-widest">Type</th>
                                    <th className="p-5 text-[#68BA7F] font-bold uppercase text-[10px] tracking-widest">Timestamp</th>
                                </tr>
                            </thead>
                            <tbody>
                                {systemLogs.map((log) => (
                                    <tr key={log._id} className="border-b border-black/5 hover:bg-black/30 transition-colors">
                                        <td className="p-5">
                                            <div className="font-bold text-[#68BA7F]">{log.user?.name || 'Unknown'}</div>
                                            <div className="text-[10px] text-[#547792]">{log.user?.role}</div>
                                        </td>
                                        <td className="p-5">
                                            <p className="text-xs font-medium text-[#68BA7F]">{log.action}</p>
                                        </td>
                                        <td className="p-5">

    ...
    return (
        <div>
            ...
            <button
                ...
                disabled={loading}
                className="w-full p-4 bg-[#213448] text-white rounded-xl font-bold btn-premium hover:bg-[#213448]"
            >
                {loading ? "Sending..." : "Send OTP"}
            </button>
            <p className="text-center mt-6">
                <span onClick={() => setView("login")} className="text-[#213448] cursor-pointer font-bold underline">Back to Login</span>
            </p>
        </div>
    );
};
...
export const ResetPassword = ({ email, otp, setOtp, newPassword, setNewPassword, loading, handleResetPassword, mobile }) => {
    return (
        <div>
            <p className="text-center text-xs text-[#213448]/60 mb-6 font-bold uppercase tracking-widest border-b border-[#547792]/20 pb-2">
                Resetting for: {email}
                {mobile && <span className="block mt-1 text-[#213448]">Configured Mobile: {mobile}</span>}
            </p>

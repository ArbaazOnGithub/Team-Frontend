    ...
    return (
        <div>
            <div className="relative group w-24 h-24 mx-auto mb-6">
                <img src={imagePreview || "https://ui-avatars.com/api/?name=User&background=68BA7F&color=fff&size=150"} onError={(e) => e.target.src = "https://ui-avatars.com/api/?name=User&background=68BA7F&color=fff"} className="w-full h-full rounded-full object-cover border-4 border-[#68BA7F] shadow-md" alt="Preview" />
            ...
            <button
                ...
                disabled={loading}
                className="w-full p-4 bg-[#68BA7F] text-white rounded-xl font-bold btn-premium hover:bg-[#68BA7F] hover:shadow-lg"
            >
                {loading ? "Creating Account..." : "Register"}
            </button>
            <p className="text-center mt-5 text-sm text-[#68BA7F]/60 font-black">
                Already have an account? <span onClick={() => { setView("login"); resetForms(); }} className="text-[#68BA7F] cursor-pointer underline underline-offset-4 hover:text-[#68BA7F] transition-colors">Login</span>
            </p>
        </div>
    );
};
